        ...
        <ul className="flex items-center justify-start gap-4 my-5 overflow-x-scroll md:overflow-x-hidden scroll-smooth scrollbar-none">
          {FoodTags.map((tag, index) => (
            <li
              key={index}
              onClick={() =>
                dispatch(StateSliceAction.setTagsActive({ index: tag.name }))
              }
              className={`${
                activeIndex === tag.name ? "bg-green-500 text-white" : ""
              } text-lg px-3 py-1 cursor-pointer font-medium text-black bg-gray-200 hover:text-white hover:bg-green-500 rounded-lg`}
            >
              {tag.name}
            </li>
          ))}
